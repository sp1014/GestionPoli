...
class CellBookmark {
  ...
  resolve(doc) {
    ...
    if (
      ...
      return new CellSelection($anchorCell, $headCell);
    else return prosemirrorState.Selection.near($headCell, 1);
  }
}
...
const fixTablesKey = new prosemirrorState.PluginKey('fix-tables');
...
function goToNextCell(direction) {
  return function (state, dispatch) {
    ...
    if (dispatch) {
      ...
      dispatch(
        state.tr
          .setSelection(prosemirrorState.TextSelection.between($cell, moveCellForward($cell)))
          .scrollIntoView(),
      );
    }
    return true;
